...
export default function DailyRecordForm({ date, initialRecord, onSave, readOnly }) {
  ...
  // Load parties for the debit dropdown
  useEffect(() => {
  ...
  const handleDebitChange = (index, field, value) => {
    const updated = [...debitedCash];
    updated[index] = {
      ...updated[index],
      [field]: value,
    };
    setDebitedCash(updated);
  };

  const handleDebitPartyChange = (index, partyId) => {
  ...
  const handleRemoveDebit = (index) => {
    setDebitedCash(debitedCash.filter((_, i) => i !== index));
  };
        ...
        <div className="border-t border-gray-100 pt-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {!isFormDisabled && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-orange-700 border border-orange-200 rounded-md hover:bg-orange-50 transition-colors"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                  <path fillRule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clipRule="evenodd" />
                </svg>
          ...
          {debitedCash.length === 0 ? (
            ...
            <div className="space-y-3">
              ...
              {debitedCash.map((debit, index) => (
                <div key={index} className="flex flex-col md:flex-row gap-3 md:gap-4 items-start md:items-center bg-orange-50/30 md:bg-transparent p-3 md:p-0 rounded-lg md:rounded-none border md:border-0 border-orange-100">
                  <div className="w-full md:w-1/3">
                    ...
                    <input
                      ...
                      min="0"
                      placeholder="0.00"
                      value={debit.amount === 0 ? '' : debit.amount}
                      disabled={isFormDisabled}
                      onChange={(e) => handleDebitChange(index, 'amount', parseFloat(e.target.value) || 0)}
                      className="w-full h-10 px-3 text-sm border border-adani-border rounded-md focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:bg-gray-100 disabled:cursor-not-allowed font-medium text-gray-900 bg-white"
                    />
                  </div>
                  ...
                  <div className="w-full flex-1">
                    ...
                    <SearchDropdown
                      ...
                      value={debit.partyId}
                      onChange={(partyId) => handleDebitPartyChange(index, partyId)}
